    ...
    button.addEventListener('click', async () => {
      ...
      try {
        ...
        if (code.includes('too-many-requests')) {
          message.textContent = 'Juda ko‘p urinish bo‘ldi. Birozdan keyin qayta urinib ko‘ring.';
        } else if (code.includes('invalid-email')) {
          message.textContent = 'Email manzilini tekshiring.';
        } else {
          message.textContent = 'Parolni tiklashda xatolik yuz berdi. Qayta urinib ko‘ring.';
        }
        message.style.display = 'block';
