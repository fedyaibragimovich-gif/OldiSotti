...
import {
  Home, MessageSquare, Plus, Heart, User, Layers, LogOut, X,
  ChevronRight, ChevronDown, ShieldCheck, Loader2, Settings
} from 'lucide-react';
...
export const BottomNav: React.FC<BottomNavProps> = React.memo(({
  lang, unreadCount, favoritesCount, onHomeClick, onMessagesClick,
  onPostAdClick, onFavoritesClick, onMyAdsClick, onOpenAdmin, onOpenAuth
}) => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (profileRef.current && !profileRef.current.contains(e.target as Node)) setProfileModalOpen(false);
    };
  ...
  const navLabels = {
    uz: { home: 'Asosiy', messages: 'Xabarlar', postAd: "E'lon", favorites: 'Saralangan', profile: 'Profil', myAds: "Mening e'lonlarim", logout: 'Akkauntdan chiqish', login: 'Kirish', register: "Ro'yxatdan o'tish", settings: 'Sozlamalar', question: 'Akkauntdan chiqmoqchimisiz?', cancel: 'Bekor qilish', confirm: 'Ha, chiqish', logoutError: 'Chiqishda xatolik yuz berdi. Qayta urinib ko‘ring.' },
    ru: { home: 'Главная', messages: 'Сообщения', postAd: 'Подать', favorites: 'Избранное', profile: 'Профиль', myAds: 'Мои объявления', logout: 'Выйти из аккаунта', login: 'Войти', register: 'Регистрация', settings: 'Настройки', question: 'Выйти из аккаунта?', cancel: 'Отмена', confirm: 'Да, выйти', logoutError: 'Не удалось выйти. Повторите попытку.' },
    oz: { home: 'Асосий', messages: 'Хабарлар', postAd: 'Эълон', favorites: 'Сараланган', profile: 'Профил', myAds: 'Менинг эълонларим', logout: 'Аккаунтдан чиқиш', login: 'Кириш', register: 'Рўйхатдан ўтиш', settings: 'Созламалар', question: 'Аккаунтдан чиқмоқчимисиз?', cancel: 'Бекор қилиш', confirm: 'Ҳа, чиқиш', logoutError: 'Чиқишда хатолик юз берди. Қайта уриниб кўринг.' }
  }[lang] || { home: 'Asosiy', messages: 'Xabarlar', postAd: "E'lon", favorites: 'Saralangan', profile: 'Profil', myAds: "Mening e'lonlarim", logout: 'Akkauntdan chiqish', login: 'Kirish', register: "Ro'yxatdan o'tish", settings: 'Sozlamalar', question: 'Akkauntdan chiqmoqchimisiz?', cancel: 'Bekor qilish', confirm: 'Ha, chiqish', logoutError: 'Chiqishda xatolik yuz berdi. Qayta urinib ko‘ring.' };

  const openAuth = (mode: 'login' | 'register') => {
    setProfileModalOpen(false);
    if (onOpenAuth) onOpenAuth(mode);
    else { setAuthMode(mode); setAuthModalOpen(true); }
  };
  const handleLogout = async () => {
    if (isLoggingOut || !confirmLogout) return;
    try {
  ...
  return (
    <>
      {!onOpenAuth && <AuthModal isOpen={authModalOpen} onClose={() => setAuthModalOpen(false)} lang={lang} initialMode={authMode} />}
      ...
      {profileModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/60 transition-opacity">
          <div ref={profileRef} role="dialog" aria-modal="true" aria-label={navLabels.profile} className="w-full max-w-md max-h-[calc(100dvh-1rem)] overflow-y-auto bg-white dark:bg-slate-900 rounded-t-3xl sm:rounded-3xl border border-slate-200 dark:border-slate-800 p-5 shadow-2xl animate-in slide-in-from-bottom duration-200 text-slate-900 dark:text-white">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0"><h4 className="font-bold text-base truncate">{displayName}</h4><p className="text-xs text-slate-500 dark:text-slate-400 truncate">{accountIdentifier || 'Akkauntga kiring'}</p></div>
              </div>
              <button type="button" aria-label="Close" onClick={() => setProfileModalOpen(false)} className="p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 cursor-pointer"><X size={18} /></button>
            </div>
            ...
            {!user ? (
              ...
              <>
                ...
                <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                  <button id="bottom-profile-settings-btn" type="button" aria-expanded={settingsOpen} aria-controls="bottom-profile-settings-panel" onClick={() => { setSettingsOpen(open => !open); setConfirmLogout(false); setLogoutFailed(false); }} className="w-full flex items-center justify-between gap-2 px-3 py-3 text-xs font-medium text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800/70 rounded-xl cursor-pointer">
                    <span className="flex items-center gap-2"><Settings size={15} />{navLabels.settings}</span><ChevronDown size={15} className={`transition-transform ${settingsOpen ? 'rotate-180' : ''}`} />
                  </button>
                  {settingsOpen && (
