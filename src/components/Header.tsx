    ...
    <header className="sticky top-0 z-30 bg-white dark:bg-slate-900 border-b border-slate-200/90 dark:border-slate-800 shadow-2xs w-full max-w-full transition-colors duration-200 transform-gpu">
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-2 sm:px-4 py-2 sm:py-3 w-full gap-1.5 sm:gap-2">
        ...
        <div className="flex items-center space-x-1.5 sm:space-x-3 shrink-0">
          ...
          >
            {/* Distinctive modern exchange mark */}
        ...
        {/* Right actions: Currency (UZS / USD ($)), Language, Mode toggle, & Favorites */}
        <div className="flex items-center space-x-1 sm:space-x-2.5 shrink-0">
          {/* Currency Switcher: UZS and USD ($) */}
          <div className="flex items-center space-x-0.5 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-xl border border-slate-200/80 dark:border-slate-700 text-[10px] sm:text-xs">
            <button
              ...
              className={`rounded-lg px-1.5 sm:px-2.5 py-1 font-bold transition-all cursor-pointer ${
                currency === 'UZS'
            ...
            {langDropdownOpen && (
              <div className="absolute right-0 mt-1.5 w-40 rounded-xl bg-white dark:bg-slate-900 py-1.5 shadow-xl border border-slate-200 dark:border-slate-800 z-50 animate-in fade-in zoom-in-95 duration-150">
                <button
                  ...
                  className={`block w-full px-3 py-1.5 text-left text-xs font-semibold cursor-pointer ${
                    lang === 'uz'
          ...
          {/* Primary Top Mode Change Button (Light / Dark) */}
          {onToggleDarkMode && (
            <button
              id="theme-toggle-main-nav-btn"
              type="button"
              onClick={onToggleDarkMode}
              className="flex items-center gap-1.5 rounded-xl px-2.5 sm:px-3 py-1.5 text-xs sm:text-sm font-bold bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-200/80 dark:hover:bg-slate-700 transition-all cursor-pointer border border-slate-200/80 dark:border-slate-700 shadow-2xs"
              title={darkMode ? t.dayMode : t.nightMode}
              aria-label={t.themeToggle}
            >
              {darkMode ? (
                <>
                  <Sun size={16} className="text-amber-400 shrink-0" />
                  <span className="hidden md:inline text-amber-500 dark:text-amber-400 font-semibold">{t.dayMode}</span>
                </>
              ) : (
                <>
                  <Moon size={16} className="text-indigo-600 dark:text-indigo-400 shrink-0" />
                  <span className="hidden md:inline text-slate-700 dark:text-slate-200 font-semibold">{t.nightMode}</span>
                </>
              )}
            </button>
          )}
          ...
          <div
            id="cloud-db-status-badge"
            className={`hidden md:flex items-center gap-1.5 rounded-xl px-2 sm:px-2.5 py-1.5 text-xs font-semibold border transition-all select-none ${
              isDbConnected
            ...
            title={
              isDbConnected
                ? "Firestore bulutli ma'lumotlar bazasi ulangan va real-vaqt rejimida sinxronlanmoqda"
                : "Ma'lumotlar bazasi ulanmoqda..."
            }
          >
            <span
              className={`w-2 h-2 rounded-full ${
                isDbConnected ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'
              }`}
            />
          ...
          {onOpenAdmin && (
            <button
              ...
              type="button"
              onClick={onOpenAdmin}
              className="flex items-center gap-1.5 rounded-xl px-2 sm:px-2.5 py-1.5 text-xs sm:text-sm font-bold bg-indigo-50 dark:bg-indigo-950/70 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/80 border border-indigo-200/80 dark:border-indigo-800/80 transition-all cursor-pointer shadow-2xs"
              title={t.adminPanel}
          ...
          {/* Favorites (Visible on desktop/tablet; on mobile it resides in the bottom navigation dock) */}
          <button
            id="header-favorites-btn"
            type="button"
            onClick={onOpenFavorites}
            className="relative hidden md:flex items-center gap-1.5 rounded-xl px-2 sm:px-2.5 py-1.5 text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-200 hover:text-rose-500 dark:hover:text-rose-400 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200/70 dark:hover:bg-slate-700 transition-colors cursor-pointer border border-slate-200/80 dark:border-slate-700"
            title={t.favorites}
          >
      ...
      {/* Mobile Quick Categories Swipeable Bar */}
      <div
        id="header-quick-categories-bar"
        ref={quickBarRef}
        className="md:hidden border-t border-slate-200/70 dark:border-slate-800 bg-slate-50/95 dark:bg-slate-900/95 backdrop-blur-md px-2 py-1.5 overflow-x-auto [&::-webkit-scrollbar]:hidden flex items-center gap-1.5 w-full transition-colors"
        style={{ WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none' }}
        aria-label="Quick Categories"
      >
        {/* All Categories Option */}
        <button
          id="header-quick-cat-all"
          type="button"
          onClick={() => {
            onSelectCategory?.('');
            document.getElementById('listings-feed-anchor')?.scrollIntoView({ behavior: 'smooth' });
          }}
          className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs whitespace-nowrap shrink-0 transition-all cursor-pointer ${
            !selectedCategoryId
              ? 'bg-indigo-600 text-white font-bold shadow-xs'
              : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-slate-700/80 hover:bg-slate-100 dark:hover:bg-slate-700 font-medium'
          }`}
        >
