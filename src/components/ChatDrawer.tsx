              ...
              {conversations.map((conv) => {
                ...
                return <button key={conv.id} onClick={() => onSelectChat(conv.id)} className={`flex w-full min-w-0 items-start gap-2.5 p-3 text-left transition-colors cursor-pointer ${isActive ? 'bg-white dark:bg-slate-900 border-l-4 border-indigo-600 shadow-2xs' : 'hover:bg-slate-100 dark:hover:bg-slate-900/60'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex min-w-0 items-center justify-between gap-2"><span className="text-xs font-bold text-slate-900 dark:text-white truncate">{conv.sellerName}</span><span className="text-[10px] text-slate-400 shrink-0">{conv.lastUpdated}</span></div>
          ...
          {activeConv ? <div className="flex-1 min-w-0 flex flex-col bg-white dark:bg-slate-900 overflow-hidden">
            <div className="flex min-w-0 items-center justify-between gap-2 p-2.5 sm:p-3 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/40 shrink-0">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0"><h4 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white truncate">{activeConv.sellerName}</h4><span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium flex items-center gap-1"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />{t.sellerOnline}</span></div>
              </div>
